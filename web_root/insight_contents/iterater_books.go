...
const (
	...
	EXT_MD            string = ".md"
	DirPrefix         string = "../../public/insight_contents"
	PathInsightComponent string = "../../src/components/Insight.vue"
...
func GetArticleMetadata(htmlPaths []string) []ArticleMetadata {
	...
	for _, htmlPath := range htmlPaths {
		...
		// Get Uri
		uri := strings.Replace(htmlPath, "../../public/insight_contents/", "/insight/", 1)
		...
		labelFrom += len("label_")
		labelTo := bytes.Index(htmlSource, []byte(".png"))

		// Get bookURL
		bookURLFrom := bytes.Index(htmlSource, []byte("https"))
		bookURLTo := bytes.Index(htmlSource, []byte("\"><img"))

		metadata = append(metadata, ArticleMetadata{
			Title:		string(htmlSource[titleFrom:titleTo]),
			Uri:       	uri,
			Date:      	date,
			Label:		string(htmlSource[labelFrom:labelTo]),
			BookURL:	string(htmlSource[bookURLFrom:bookURLTo]),
		})
	}
...
func insertJSONToInsightComponent(json string) {
	componentSourceBytes, err := ioutil.ReadFile(PathInsightComponent)
	if err != nil {
		log.Fatal(err)
	}
	...
	ioutil.WriteFile(PathInsightComponent, []byte(componentSource), 0644)
}

func insertJSONToVueConfigJs(json string) {
	json = strings.Replace(json, "[", "\"/insight/\",", 1)
	...
	from := strings.Index(vueConfigJsSource[i:], "/blog/2018/12/17/블로그-오픈/\",")
	from += len("/blog/2018/12/17/블로그-오픈/\",")
	from += i
...
func main() {
	...
	insertJSONToInsightComponent(string(b))

	//prerendering
	for _, articlePath := range articles {
		uri := strings.Replace(articlePath, "../../public/insight_contents/", "/insight/", 1)
		uri = strings.Replace(uri, ".html", "/", 1)
